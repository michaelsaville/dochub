...
/**
 * HMAC verification for inter-app BFF calls from the client portal.
 *
 * Signing scheme (matches TicketHub's BFF — intentional, same caller):
 ...
 *
 * Replay window is ±5 minutes. Raw body (not parsed JSON) is what's
 * signed so both sides compare bytes without worrying about key
 * ordering.
 */
