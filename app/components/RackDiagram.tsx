...
export default function RackDiagram({ racks, locations, networkDevices, assets, clientId, onRacksChange }: Props) {
  ...
  const [slotForm, setSlotForm] = useState({ heightU: "1", label: "", networkDeviceId: "", assetId: "", notes: "" })
    ...
    try {
      ...
      if (res.ok) {
        ...
        onRacksChange(racks.map(r => r.id === rackId
          ? { ...r, slots: [...r.slots, created].sort((a, b) => a.startU - b.startU) }
          : r
        ))
        setSlotForm({ heightU: "1", label: "", networkDeviceId: "", assetId: "", notes: "" })
  ...
  function getSlotColor(slot: RackSlot): string {
  ...
  function getSlotName(slot: RackSlot): string {
  ...
  function getSlotSubtitle(slot: RackSlot): string {
    const dev = slot.networkDevice ?? slot.asset
    if (!dev) return ""
    const parts = [dev.make, dev.model].filter(Boolean)
    return parts.join(" ")
  }

  function renderRack(rack: Rack) {
    const U_HEIGHT = 28 // px per U unit
    const occupiedUs = new Set<number>()
    const slotMap = new Map<number, RackSlot>()

    rack.slots.forEach(slot => {
      for (let u = slot.startU; u < slot.startU + slot.heightU; u++) {
        occupiedUs.add(u)
      }
      slotMap.set(slot.startU, slot)
    })
    ...
    while (u <= rack.totalU) {
      ...
      if (slot) {
        rows.push({ type: "slot" as const, slot, u })
        u += slot.heightU
      } else {
        rows.push({ type: "empty" as const, u })
        u++
      }
    }

    return (
      <div key={rack.id} style={{ marginBottom: "24px" }}>
        {/* Rack header */}
          ...
          {editingRack === rack.id ? (
            <div style={{ flex: 1, background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-secondary)", borderRadius: "10px", padding: "16px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 80px", gap: "12px", marginBottom: "12px" }}>
                ...
                <div>
                  <label style={label}>Total U</label>
            ...
            <>
              <div>
                ...
                <span style={{ fontSize: "13px", color: "var(--color-text-muted)", marginLeft: "10px" }}>{rack.location.name} · {rack.totalU}U · {rack.slots.length} device{rack.slots.length !== 1 ? "s" : ""}</span>
              </div>
        ...
        {/* Rack visual */}
        <div style={{ display: "flex", border: "2px solid #334155", borderRadius: "8px", overflow: "hidden", background: "#0f172a", maxWidth: "680px" }}>
          {/* U number column */}
          <div style={{ width: "32px", background: "#1e293b", borderRight: "1px solid #334155", flexShrink: 0 }}>
            {rows.map(row => (
              <div key={row.u} style={{
                height: row.type === "slot" ? `${row.slot.heightU * U_HEIGHT}px` : `${U_HEIGHT}px`,
            ...
            {rows.map(row => {
              if (row.type === "slot") {
                ...
                return (
                  <div
                    key={slot.id}
                    style={{
                      height: `${slot.heightU * U_HEIGHT}px`,
                    ...
                    {isEditing ? (
                      <div style={{ padding: "10px 12px" }}>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginBottom: "8px" }}>
                          <div>
                            <label style={{ ...label, color: "#94a3b8" }}>Label</label>
                            <input value={slotEditForm.label ?? ""} onChange={e => setSlotEditForm((f: any) => ({ ...f, label: e.target.value }))} placeholder="Optional override" style={{ ...input, background: "#0f172a", color: "#e2e8f0", borderColor: "#334155", fontSize: "12px", padding: "4px 8px" }} />
                          </div>
                          <div>
                            <label style={{ ...label, color: "#94a3b8" }}>Notes</label>
                            <input value={slotEditForm.notes ?? ""} onChange={e => setSlotEditForm((f: any) => ({ ...f, notes: e.target.value }))} style={{ ...input, background: "#0f172a", color: "#e2e8f0", borderColor: "#334155", fontSize: "12px", padding: "4px 8px" }} />
                          </div>
                          <div>
                            <label style={{ ...label, color: "#94a3b8" }}>Network Device</label>
                            <select value={slotEditForm.networkDeviceId ?? ""} onChange={e => setSlotEditForm((f: any) => ({ ...f, networkDeviceId: e.target.value, assetId: "" }))} style={{ ...input, background: "#0f172a", color: "#e2e8f0", borderColor: "#334155", fontSize: "12px", padding: "4px 8px" }}>
                              <option value="">None</option>
                              {networkDevices.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                            </select>
                          </div>
                          <div>
                            <label style={{ ...label, color: "#94a3b8" }}>Asset</label>
                            <select value={slotEditForm.assetId ?? ""} onChange={e => setSlotEditForm((f: any) => ({ ...f, assetId: e.target.value, networkDeviceId: "" }))} style={{ ...input, background: "#0f172a", color: "#e2e8f0", borderColor: "#334155", fontSize: "12px", padding: "4px 8px" }}>
                              <option value="">None</option>
                        ...
                        <div style={{ display: "flex", gap: "6px" }}>
                          <button onClick={() => updateSlot(rack.id, slot.id)} disabled={savingSlot} style={{ fontSize: "11px", fontWeight: 500, padding: "4px 10px", borderRadius: "6px", border: "none", background: "#e2e8f0", color: "#0f172a", cursor: "pointer" }}>Save</button>
                          <button onClick={() => setEditingSlot(null)} style={{ fontSize: "11px", padding: "4px 10px", borderRadius: "6px", border: "1px solid #334155", background: "transparent", cursor: "pointer", color: "#94a3b8" }}>Cancel</button>
                          <button onClick={() => deleteSlot(rack.id, slot.id)} style={{ fontSize: "11px", padding: "4px 10px", borderRadius: "6px", border: "none", background: "#7f1d1d", color: "#fca5a5", cursor: "pointer", marginLeft: "auto" }}>Remove</button>
                        </div>
                      </div>
                    ) : (
                      <div style={{ padding: "0 10px", height: "100%", display: "flex", flexDirection: "column", justifyContent: "center" }}>
                        ...
                        {isHovered && (
                          <button
                            ...
                            style={{ position: "absolute", right: "8px", top: "50%", transform: "translateY(-50%)", fontSize: "11px", padding: "3px 8px", borderRadius: "5px", border: "1px solid #475569", background: "#1e293b", color: "#94a3b8", cursor: "pointer" }}
                          >
                            Edit
                          </button>
                        )}
              ...
              return (
                <div
                  ...
                  onClick={() => { if (!isAddingHere) { setAddingSlotTo({ rackId: rack.id, startU: row.u }); setSlotForm({ heightU: "1", label: "", networkDeviceId: "", assetId: "", notes: "" }) } }}
                >
        ...
        {addingSlotTo?.rackId === rack.id && (
          <div style={{ marginTop: "8px", background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-secondary)", borderRadius: "10px", padding: "16px", maxWidth: "680px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "80px 1fr 1fr", gap: "12px", marginBottom: "12px" }}>
              <div>
                <label style={label}>Height (U)</label>
              ...
              <div>
                <label style={label}>Network Device</label>
              ...
              <div style={{ gridColumn: "1 / -1" }}>
                <label style={label}>Label (optional — overrides device name in diagram)</label>
      ...
      {showAddRack && (
        <div style={{ background: "var(--color-background-secondary)", border: "0.5px solid var(--color-border-secondary)", borderRadius: "10px", padding: "20px", marginBottom: "20px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 80px", gap: "12px", marginBottom: "12px" }}>
            ...
            <div>
              <label style={label}>Size (U)</label>
