  ...
  return (
    <AppShell>
      <div style={{ padding: "32px", maxWidth: "820px" }}>
        ...
        {/* Active run banner */}
        {activeRun && (
          <div style={{ background: "rgba(61,111,255,0.08)", border: "0.5px solid rgba(61,111,255,0.3)", borderRadius: "10px", padding: "14px 20px", marginBottom: "20px" }}>
        ...
        <div style={{ marginBottom: "20px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap", marginBottom: "8px" }}>
            ...
            {runbook.client && (
              <button onClick={() => router.push(`/clients/${runbook.client.id}`)} style={{ fontSize: "11px", padding: "2px 8px", borderRadius: "10px", background: "rgba(61,111,255,0.13)", color: "var(--accent)", border: "none", cursor: "pointer" }}>
                {runbook.client.name} ↗
              </button>
            )}
          </div>
        ...
        {totalSteps > 0 && (
          <div style={{ marginBottom: "32px" }}>
            ...
            <div style={{ height: "4px", background: "var(--color-background-hover)", borderRadius: "2px", marginBottom: "16px", overflow: "hidden" }}>
              <div style={{ height: "100%", width: `${totalSteps ? (completedCount / totalSteps) * 100 : 0}%`, background: allDone ? "var(--accent2)" : "var(--accent)", borderRadius: "2px", transition: "width 0.3s ease" }} />
            </div>

            {allDone && !activeRun && (
              <div style={{ padding: "12px 16px", background: "rgba(0,212,170,0.13)", border: "1px solid rgba(0,212,170,0.27)", borderRadius: "8px", fontSize: "14px", color: "var(--accent2)", fontWeight: 500, marginBottom: "16px" }}>
                All steps complete ✓
              </div>
            )}
            ...
            {showPastRuns && (
              <div style={{ marginTop: "12px" }}>
                {loadingRuns ? (
                  ...
                  const statusColor = run.status === "COMPLETED" ? "var(--accent2)" : run.status === "ABANDONED" ? "var(--danger)" : "var(--accent)"
