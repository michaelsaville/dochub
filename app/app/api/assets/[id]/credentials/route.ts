...
// POST { childIds: string[] } — set Credential.assetId = id for each.
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    for (const cid of childIds) {
      try {
        await assertChildBelongsToClient("credential", cid, clientId)
        await assertLinkable("credential", cid)
        const before = await prisma.credential.findUnique({ where: { id: cid }, select: { assetId: true } })
        if (before?.assetId === id) { results.push({ id: cid, status: "skipped", reason: "already linked" }); continue }
        await prisma.credential.update({ where: { id: cid }, data: { assetId: id } })
    ...
    console.error("[api/assets/:id/credentials POST]", e)
    return NextResponse.json({ error: "Failed to link" }, { status: 500 })
  }
}

// DELETE ?childId= — null out Credential.assetId
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const cred = await prisma.credential.findUnique({ where: { id: childId }, select: { assetId: true } })
    if (!cred || cred.assetId !== id) return NextResponse.json({ error: "Not linked" }, { status: 404 })
    await prisma.credential.update({ where: { id: childId }, data: { assetId: null } })
    return NextResponse.json({ ok: true })
  } catch (e) {
    console.error("[api/assets/:id/credentials DELETE]", e)
