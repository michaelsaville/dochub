...
// POST { childIds: string[] } — set runbook.assetId = id for each.
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    for (const cid of childIds) {
      try {
        await assertChildBelongsToClient("runbook", cid, clientId)
        const before = await prisma.runbook.findUnique({ where: { id: cid }, select: { assetId: true } })
        if (before?.assetId === id) { results.push({ id: cid, status: "skipped", reason: "already linked" }); continue }
        await prisma.runbook.update({ where: { id: cid }, data: { assetId: id } })
    ...
    console.error("[api/assets/:id/runbooks POST]", e)
    return NextResponse.json({ error: "Failed to link" }, { status: 500 })
  }
}

// DELETE ?childId= — null out runbook.assetId
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const row = await prisma.runbook.findUnique({ where: { id: childId }, select: { assetId: true } })
    if (!row || row.assetId !== id) return NextResponse.json({ error: "Not linked" }, { status: 404 })
    await prisma.runbook.update({ where: { id: childId }, data: { assetId: null } })
    return NextResponse.json({ ok: true })
  } catch (e) {
    console.error("[api/assets/:id/runbooks DELETE]", e)
