...
// POST { childIds: string[] } — set clientDocument.assetId = id for each.
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    for (const cid of childIds) {
      try {
        await assertChildBelongsToClient("clientDocument", cid, clientId)
        const before = await prisma.clientDocument.findUnique({ where: { id: cid }, select: { assetId: true } })
        if (before?.assetId === id) { results.push({ id: cid, status: "skipped", reason: "already linked" }); continue }
        await prisma.clientDocument.update({ where: { id: cid }, data: { assetId: id } })
    ...
    console.error("[api/assets/:id/documents POST]", e)
    return NextResponse.json({ error: "Failed to link" }, { status: 500 })
  }
}

// DELETE ?childId= — null out clientDocument.assetId
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const row = await prisma.clientDocument.findUnique({ where: { id: childId }, select: { assetId: true } })
    if (!row || row.assetId !== id) return NextResponse.json({ error: "Not linked" }, { status: 404 })
    await prisma.clientDocument.update({ where: { id: childId }, data: { assetId: null } })
    return NextResponse.json({ ok: true })
  } catch (e) {
    console.error("[api/assets/:id/documents DELETE]", e)
