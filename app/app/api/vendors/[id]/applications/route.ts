...
// POST { childIds: string[] } — set Application.vendorId = id for each.
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    for (const cid of childIds) {
      try {
        const existing = await prisma.application.findUnique({ where: { id: cid }, select: { vendorId: true } })
        if (!existing) { results.push({ id: cid, status: "error", reason: "application not found" }); continue }
        if (existing.vendorId === id) { results.push({ id: cid, status: "skipped", reason: "already linked" }); continue }
        await prisma.application.update({ where: { id: cid }, data: { vendorId: id } })
    ...
    console.error("[api/vendors/:id/applications POST]", e)
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const row = await prisma.application.findUnique({ where: { id: childId }, select: { vendorId: true } })
    if (!row || row.vendorId !== id) return NextResponse.json({ error: "Not linked" }, { status: 404 })
    await prisma.application.update({ where: { id: childId }, data: { vendorId: null } })
    return NextResponse.json({ ok: true })
  } catch (e) {
    console.error("[api/vendors/:id/applications DELETE]", e)
