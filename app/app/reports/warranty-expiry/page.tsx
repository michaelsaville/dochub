...
export default function WarrantyExpiryReport() {
  ...
  function warrantyStatus(date: string | null): { label: string; color: string } {
    ...
    if (d < cutoff) return { label: `Expires ${d.toLocaleDateString()}`, color: "#f59e0b" }
      ...
      {generated && (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "12px", marginBottom: "28px" }}>
            ...
            <ReportStat label={`Expiring within ${days} days`} value={expiringSoon.length} color={expiringSoon.length > 0 ? "#f59e0b" : undefined} />
            <ReportStat label="Valid" value={valid.length} color={valid.length > 0 ? "#22c55e" : undefined} />
          </div>
