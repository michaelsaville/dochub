// Seed the DocHub starter Template Library (documents + SOPs).
//
// Idempotent by `seedKey`: re-running only creates what's missing and restores
// archived seeds. Admin edits to seeded templates are PRESERVED unless you pass
// --force (which overwrites content back to the seed default).
//
// Run from the app root:
//   DATABASE_URL=... node scripts/seed-templates.mjs
//   DATABASE_URL=... node scripts/seed-templates.mjs --force
//
// This script is intentionally self-contained (no TS imports) so it runs under
// plain node. It mirrors lib/template-seeds.ts, which POST /api/templates/reseed
// uses for the same effect from inside the app.
...
try {
  console.log(`Seeding template library${FORCE ? ' (--force: overwriting content)' : ''}…`)
  ...
  for (const s of SEEDS) {
    ...
    if (!existing) {
      ...
      created++
      console.log(`  + created  ${s.seedKey}`)
    } else {
      await prisma.template.update({
        where: { seedKey: s.seedKey },
        data: FORCE ? { ...fields, isArchived: false } : { isArchived: false },
      })
      if (FORCE) { updated++; console.log(`  ~ updated  ${s.seedKey}`) }
      else if (existing.isArchived) { restored++; console.log(`  ^ restored ${s.seedKey}`) }
      else console.log(`  = exists   ${s.seedKey}`)
    }
  }

  console.log(`\nDone. created=${created} restored=${restored} updated=${updated} total=${SEEDS.length}`)
} catch (e) {
